...
# defining function to pass in the terminal
...
def download_data():
    ...
    if "hymenoptera_data" not in os.listdir("../data"):
...
def get_dataloaders():
    ...
    return {"class_names": class_names, "dataset_sizes": dataset_sizes, "dataloaders": dataloaders}
...
def create_model(weights=None, epochs=5, optimizer='sgd'):
    ...
    if weights:
        if weights not in os.listdir("../model"):
            print("The weights do not exists in the current folder")
    ...
    if optimizer == 'sgd':
        ...
        print("Sorry I can't use that optimizer")
        return

    print(f"Using optimizer: {optimizer_conv}")
    ...
    torch.save(model_conv, "../models/model.pkl")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--download-data',
                        help="Download the standard data", action='store_true')
